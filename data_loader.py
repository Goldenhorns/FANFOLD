...
def get_ad_dataset_TU(args, split, need_str_enc=True, use_node_attr=True):
    DS = args.DS
    path = osp.join(osp.dirname(osp.realpath(__file__)), '.', 'data', DS)

    if DS in ['IMDB-BINARY', 'REDDIT-BINARY', 'COLLAB']:
        dataset = TUDataset(path, name=DS, transform=(Constant(1, cat=False)), use_node_attr=use_node_attr)
    else:
        dataset = TUDataset(path, name=DS, use_node_attr=use_node_attr)
    ...
    meta = {'num_feat':dataset_num_features, 'num_train':len(data_train), 'num_test':len(data_test), 'lable':len(label_list)}
